...
namespace PrimitiveMotionLevel {
  ...
  PrimitiveCommand::PrimitiveCommand(const std::string& name) :
    ...
    supportCOM_(false),
    fixedFrame_(false),
    fixedFrameChanged_(false)
  {
  }

  void PrimitiveCommand::updateFromIdl(const WholeBodyMasterSlaveChoreonoidIdl::PrimitiveState& idl) {
    ...
    if(this->fixedFrame_ != idl.fixedFrame) this->fixedFrameChanged_ = true;
    else this->fixedFrameChanged_ = false;
    this->fixedFrame_ = idl.fixedFrame;
  }
